...
const serviceIconType: Record<string, string> = {
  "commercial-store":"store",restaurant:"restaurant",cafe:"cafe","convenience-store":"shop",franchise:"link",karaoke:"mic","kids-cafe":"kids","pc-room":"monitor","beauty-salon":"scissors","beauty-shop":"sparkle",gym:"dumbbell",academy:"book","study-cafe":"desk",hospital:"medical",dental:"tooth",pharmacy:"pharmacy",office:"office","retail-store":"bag","unmanned-store":"scan",mart:"cart",lodging:"bed",bathhouse:"bath",laundry:"washer",warehouse:"warehouse",factory:"factory",house:"house",apartment:"building",interior:"hammer",partial:"cut",pub:"glass",bakery:"bread","pet-shop":"paw"
};

const serviceTone: Record<string,string> = {
  "commercial-store":"blue",restaurant:"orange",cafe:"brown","convenience-store":"green",franchise:"indigo",karaoke:"purple","kids-cafe":"pink","pc-room":"cyan","beauty-salon":"rose","beauty-shop":"violet",gym:"red",academy:"blue","study-cafe":"teal",hospital:"green",dental:"sky",pharmacy:"emerald",office:"slate","retail-store":"amber","unmanned-store":"cyan",mart:"orange",lodging:"indigo",bathhouse:"sky",laundry:"blue",warehouse:"slate",factory:"charcoal",house:"green",apartment:"indigo",interior:"amber",partial:"rose",pub:"purple",bakery:"brown","pet-shop":"pink"
};
...
export default function Page(){
  return <main className="page-shell">
    ...
    <header className="list-hero"><div className="eyebrow-chip">SERVICE DIRECTORY</div><h1>업종에 따라 달라지는<br/><span className="gradient-text">철거 범위와 원상복구</span></h1><p>평수만으로 철거를 판단하기보다 업종별 설비, 마감, 폐기물, 반출 조건과 임대차 원상복구 범위를 함께 확인하세요.</p><div className="cta-row"><a className="btn btn-primary" href={inquiryUrl}>무료 현장견적 문의</a><a className="btn btn-glass" href="/estimate">견적 준비정보 6가지</a></div></header>
    <section className="section"><div className="section-heading"><div><span className="section-kicker">ALL SERVICES</span><h2>업종별 철거 서비스</h2></div><p>각 페이지에서 철거 전 확인사항, 견적에 영향을 주는 항목, 작업 순서와 폐업지원 정보를 확인할 수 있습니다.</p></div><div className="service-grid">{Object.entries(services).map(([slug,item])=>{const tone=serviceTone[slug]??"blue";return <a key={slug} className={`service-card service-directory-card tone-${tone}`} href={`/service/${slug}`}><div className="service-card-top"><span className="service-directory-icon"><ServiceIcon slug={slug}/></span><span className="service-card-arrow" aria-hidden="true">↗</span></div><strong>{item.primary}</strong><span>{item.summary}</span></a>})}</div></section>
    <section className="support-box section"><span className="section-kicker">2026 CLOSURE SUPPORT</span><h2>폐업 예정이라면 철거 전에 지원 대상도 확인하세요</h2><p>점포철거비 지원은 임차관계, 폐업일, 면적, 인정비용과 증빙에 따라 달라지며 지원에서 제외되는 경우도 있습니다. 업종별 철거정보를 확인한 뒤 공사 전에 최신 지원조건을 함께 확인하세요.</p><div className="cta-row"><a className="btn btn-primary" href="/support">2026 지원 대상·제외조건 확인</a><a className="btn btn-glass" href="/guide/demolition-estimate-checklist">견적 비교 체크리스트</a></div></section>
    <section className="final-cta"><div><span className="section-kicker">FIELD ESTIMATE</span><h2>어떤 항목을 철거해야 할지<br/>애매하다면 현장에서 확인하세요.</h2><p>업종과 면적, 층수, 설비, 반출동선, 원상복구 조건을 함께 확인하면 견적 범위를 더 구체적으로 정리할 수 있습니다.</p></div><a className="btn btn-light" href={inquiryUrl}>무료 현장견적 문의</a></section>
  </main>;
}